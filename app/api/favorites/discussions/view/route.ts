...
export async function GET(req: NextRequest) {
	try {
		...
		const total = await db.discussionFavorites.count({ where: { userId } });
		const data = await db.discussionFavorites.findMany({
			where: { userId },
			take: count,
			skip: count * (page - 1),
			include: {
		...
		return NextResponse.json({ total, hits: data.map((d) => d.Discussion) });
	} catch (err) {
		console.log(err);
